<template>
  <div>
    <div v-if="flash.success && show" class="flex items-center justify-between mb-8 max-w-3xl bg-green-500 rounded-sm">
    ...
    <div v-if="flash.error && show" class="flex items-center justify-between mb-8 max-w-3xl bg-red-500 rounded-sm">
      <div class="flex items-center">
        ...
        <div class="py-4 text-white text-sm font-medium">{{ flash.error }}</div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
