    ...
    <Transition>
      <div v-if="show" ref="modalRef" class="fixed inset-0 z-50" @click.self="emit('close')">
        <div class="flex min-h-screen items-center justify-center p-4">
          <div class="ui-modal-scrim fixed inset-0 bg-overlay" @click="emit('close')" />
          <div class="relative rounded-lg bg-surface shadow-xl" :class="`max-w-${maxWidth}`">
            <div class="ui-modal-header border-b border-line px-6 py-4 flex items-center justify-between">
              <slot name="header" />
              <button type="button" class="ml-4 text-ink-3 hover:text-ink-2" @click="emit('close')">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                </svg>
              </button>
            </div>
            <div class="ui-modal-body bg-surface px-6 py-4">
              <slot />
            </div>
          </div>
        </div>
