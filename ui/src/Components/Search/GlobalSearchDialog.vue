<template>
  <Modal :show="show" max-width="xl" @close="close">
    <template #header>
      <div class="flex items-center gap-2">
        ...
        <input
          ...
          type="search"
          class="ui-global-search-input w-full border-0 bg-transparent text-base text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-0 dark:text-gray-100"
    ...
    <div id="ui-global-search-results" role="listbox" class="-mx-4 -mb-4 max-h-96 overflow-y-auto">
      <p v-if="search.loading.value" class="px-4 py-6 text-center text-sm text-gray-400">Searching…</p>

      <p v-else-if="search.query.value.trim() !== '' && search.hits.value.length === 0" class="px-4 py-6 text-center text-sm text-gray-400">
        Nothing matches “{{ search.query.value.trim() }}”.
      </p>

      <p v-else-if="search.hits.value.length === 0" class="px-4 py-6 text-center text-sm text-gray-400">
        Type to search across the panel.
      </p>

      <template v-for="group in search.groups.value" v-else :key="group.key">
        <div class="ui-global-search-group px-4 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-gray-500">
          {{ group.label }}
          <span v-if="group.total > group.items.length" class="ml-1 font-normal normal-case text-gray-400">{{ group.items.length }} of {{ group.total }}</span>
        </div>
        <button
          ...
          :class="search.indexOf(hit) === search.activeIndex.value ? 'bg-primary-50 text-primary-900 dark:bg-primary-900/30 dark:text-primary-100' : 'text-gray-800 hover:bg-gray-50 dark:text-gray-100 dark:hover:bg-white/5'"
        ...
        >
          ...
          <span v-if="hit.details.length" class="shrink-0 text-xs text-gray-500">{{ hit.details.join(' · ') }}</span>
        </button>
      </template>
    </div>
  </Modal>
