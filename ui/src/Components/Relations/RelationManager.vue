<template>
  <div class="ui-relation-manager">
    <!-- Relation Header -->
    <div class="ui-relation-header bg-surface border-b border-line px-6 py-4">
      <div class="flex items-center justify-between">
        <div>
          <h3 class="text-lg font-semibold text-ink">{{ title }}</h3>
          <p v-if="description" class="mt-1 text-sm text-ink-2">{{ description }}</p>
        </div>
      ...
      >
        ...
        <template #actions="{ record }">
          <ActionGroup label="Actions">
            ...
            >
              <template #default>
                <svg class="w-5 h-5 text-ink-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
